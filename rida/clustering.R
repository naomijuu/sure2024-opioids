...
pre.complete <- state_dist |> 
  hclust(method = "complete") 
prescriptions.std |> 
  mutate(cluster = cutree(pre.complete, k = 3)) |> 
  ggplot(aes(x = std_brand, y = std_generic, color = cluster)) + 
  geom_point() 
pre.complete |> 
  ggdendrogram() + 
  labs(y = "Dissimilarities between clusters")

prescription_kmpp <- function(k) {
